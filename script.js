...
/* DEMO DATA (Competency Matrix) */
const competencyDemo = [
  {
...
/* DEMO DATA (Compliance Center) */
const complianceDemo = {
  domains: ["FLSA", "FMLA", "ADA", "EEO", "OSHA", "COBRA"],
  scores: [82, 76, 88, 90, 70, 65],
  issues: 7,
  highRiskDomains: 2
};
...
/* DEMO DATA (Glossary) */
const glossaryTerms = [
  { term: "Turnover Rate", definition: "The rate at which employees leave the organization over a given period." },
  { term: "Cost per Hire", definition: "Total recruiting cost divided by the number of hires in a period." },
  { term: "Absence Rate", definition: "Percentage of work time lost due to employee absences." },
  { term: "Training ROI", definition: "Return on investment for learning and development programs." },
  { term: "Competency", definition: "A measurable pattern of knowledge, skills, abilities, and behaviors." },
  { term: "Succession Planning", definition: "Process of identifying and developing future leaders." },
  { term: "Engagement", definition: "The emotional and cognitive connection employees have to their work and organization." },
  { term: "Workforce Planning", definition: "Aligning workforce supply with organizational demand over time." },
  { term: "Compliance Risk", definition: "Exposure to legal or regulatory penalties due to non-compliance." },
  { term: "Force Distribution", definition: "Segmenting performance or talent into defined tiers." }
];
...
function showSection(id, btn) {
  ...
  if (id === "competency") buildCompetencyMatrix();
  if (id === "compliance") buildComplianceCenter();
  if (id === "kpiLibrary") updateLiveKpiExamples();
...
function buildCompetencyMatrix() {
  ...
  const labels = ["Leadership", "Ethical", "Business", "Consultation", "Critical", "Communication", "Relationship"];
  const avgScores = labels.map(key => {
    const field = key.toLowerCase().replace(" ", "");
    const mappedField = {
    ...
    const vals = competencyDemo.map(r => r[mappedField]);
    return vals.reduce((a, b) => a + b, 0) / vals.length;
  });

  if (competencyCharts.radar) competencyCharts.radar.destroy();

  competencyCharts.radar = new Chart(ctx, {
    type: "radar",
    data: {
      labels,
      datasets: [{
        label: "Average Competency Level (1–5)",
        data: avgScores,
        borderColor: "#4CC9F0",
        backgroundColor: "rgba(76, 201, 240, 0.18)",
        borderWidth: 2,
    ...
    options: {
      ...
      scales: {
        r: {
          ...
          grid: { color: "rgba(255,255,255,0.12)" },
          pointLabels: { color: "#cfcfcf" },
          ticks: { display: true, stepSize: 1, color: "#cfcfcf" }
        }
      }
    }
  });
...
/* COMPLIANCE CENTER */
function buildComplianceCenter() {
  const ctx = document.getElementById("complianceRadar");
  if (!ctx) return;

  const { domains, scores, issues, highRiskDomains } = complianceDemo;

  if (complianceCharts.radar) complianceCharts.radar.destroy();

  complianceCharts.radar = new Chart(ctx, {
    type: "radar",
    data: {
      labels: domains,
      datasets: [{
        label: "Compliance Coverage (0–100)",
        data: scores,
        borderColor: "#E53935",
        backgroundColor: "rgba(229, 57, 53, 0.18)",
        borderWidth: 2,
  ...
  document.getElementById("complianceScore").textContent = avgScore.toFixed(0);
  document.getElementById("complianceIssues").textContent = issues;
  document.getElementById("complianceHighRisk").textContent = highRiskDomains;

  const list = document.getElementById("complianceList");
  list.innerHTML = "";
  domains.forEach((d, i) => {
    const li = document.createElement("li");
    li.textContent = `${d}: coverage ${scores[i]}%.`;
    if (scores[i] < 75) {
      li.textContent += " Flagged as priority for policy review.";
    }
    list.appendChild(li);
  });
}

/* KPI LIBRARY — LIVE EXAMPLES */
function updateLiveKpiExamples() {
  const list = document.getElementById("kpiLiveList");
  if (!list || !lastSummary) return;

  list.innerHTML = "";

  const headcount = lastSummary.headcount || 0;
  const attrition = lastSummary.attrition || 0;
  const highRisk = lastSummary.high_risk || 0;

  const li1 = document.createElement("li");
  li1.textContent = `Turnover Rate (demo): ${attrition.toFixed ? attrition.toFixed(1) : attrition}% based on current HR dataset.`;
  list.appendChild(li1);

  const li2 = document.createElement("li");
  li2.textContent = `Risk Concentration: ${highRisk} high-risk employees out of ${headcount}, or ${(headcount ? (highRisk / headcount) * 100 : 0).toFixed(1)}%.`;
  list.appendChild(li2);

  if (lastKnowledge) {
    const totalHeadcount = lastKnowledge.headcounts.reduce((a, b) => a + b, 0);
    const totalSales = lastKnowledge.sales.reduce((a, b) => a + b, 0);
    const salesPerEmp = totalHeadcount > 0 ? totalSales / totalHeadcount : 0;

    const li3 = document.createElement("li");
    li3.textContent = `Sales per Employee (demo): $${salesPerEmp.toFixed(0)} using Knowledge Lab dataset.`;
    list.appendChild(li3);
  }
}
...
function filterGlossary() {
  ...
  list.innerHTML = "";
  glossaryTerms
    .filter(item => item.term.toLowerCase().includes(query) || item.definition.toLowerCase().includes(query))
    .forEach(item => {
      const li = document.createElement("li");
      li.innerHTML = `<strong>${item.term}:</strong> ${item.definition}`;
      list.appendChild(li);
...
function toggleStudyMode() {
  ...
  status.textContent = `Study Mode: ${studyMode ? "ON" : "OFF"}`;

  const labels = document.querySelectorAll(".metric-label, .section-title");
  labels.forEach(label => {
    if (studyMode) {
      label.setAttribute("data-original-title", label.textContent);
      label.textContent = label.textContent + " • SHRM Focus";
    } else {
      const original = label.getAttribute("data-original-title");
...
function setActiveAddiePhase(phase) {
  ...
  const copy = {
    Analyze: {
      ...
      base:
        "Analyze uses the Core Dashboard, Competency Matrix, and Compliance Center to understand workforce health, risk, capability, and legal exposure."
    },
    Design: {
      title: "Design • What we will measure",
      base:
        "Design translates insights into structured HR metrics, SHRM KPIs, and competency targets. The HR Metrics Matrix and KPI Library define how performance will be monitored."
    },
    Develop: {
      title: "Develop • How we will model",
      base:
        "Develop builds models, simulations, and forecasts in the Knowledge Lab. This is where HR360 tests scenarios for productivity, sales, headcount, and training ROI."
    },
    Implement: {
      title: "Implement • How we deliver",
      base:
        "Implement delivers insights through dashboards, APIs, and operational workflows. HR360 connects leaders to metrics, competencies, and compliance signals."
    },
    Evaluate: {
      title: "Evaluate • What changed",
      base:
        "Evaluate compares planned vs actual outcomes, recalculates KPIs, and feeds learning back into Analyze. Force Distribution and compliance metrics show impact."
    }
  };
  ...
  if (titleEl && bodyEl) {
    ...
    if (phase === "Develop" && lastKnowledge) {
      const totalHeadcount = lastKnowledge.headcounts.reduce((a, b) => a + b, 0);
      text += ` Modeling dataset spans ${lastKnowledge.depts.length} departments and ${totalHeadcount} employees.`;
    }
    if (phase === "Evaluate" && lastKnowledge && lastSummary) {
      text += ` Use Force Distribution, KPI Library, and Compliance Center to see how interventions shift talent tiers and risk.`;
    }
...
function updateAddieFromData() {
  ...
  const li3 = document.createElement("li");
  li3.textContent = "ADDIE wheel is synchronized with dashboards, KPIs, competencies, and compliance modules.";
  list.appendChild(li3);
}
